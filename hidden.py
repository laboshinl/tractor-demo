...
# Global variables.
NUM_LABELS = 100  # The number of labels.
...
def extract_data(filename):
    ...
    for line in open(filename):
        row = line.split(",")
        labels.append(int(hashlib.md5(row[0].encode("utf-8")).hexdigest(), 16) % (10 ** 8))
        #labels.append(abs(hash(row[0])) % (10 ** 8))
